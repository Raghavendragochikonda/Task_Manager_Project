...
function renderPage() {
  switch (state.page) {
    ...
    case 'tasks':     state.taskFilter = 'all'; return renderTasks();
...
function renderTasks() {
    ...
    return `
      <div class="kanban-col">
        <div class="kanban-col-header">
          ...
          <span class="kanban-col-title"><span class="col-dot" style="background:${dotColor}"></span>${status}</span>
        </div>
        <div class="kanban-col-body">
          ${ct.length ? ct.map(taskCard).join('') : `<div class="kanban-col-empty">No tasks here</div>`}
        </div>
      </div>
    `;
  }
  ...
  return `
    <div class="filter-bar">
      ...
      <span class="filter-count">${tasks.length} task${tasks.length !== 1 ? 's' : ''}</span>
    </div>
    <div class="kanban">
      ${col('Todo',        '#94a3b8')}
